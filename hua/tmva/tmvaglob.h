...
#ifndef TMVA_TMVAGLOB_MY
#define TMVA_TMVAGLOB_MY
...
namespace TMVAGlob {
   ...
   TDirectory *GetCorrelationPlotsDir( TMVAGlob::TypeOfPlot type, TDirectory *dir=0 )
   {
    ...
    void getFileDirName( const TString fin, TString& fileDir, TString& fileName ){
        TString inputFile = fin;
        fileDir = inputFile.Remove( inputFile.Last( '/')+1, inputFile.Length()-inputFile.Last( '/') );
        fileDir = fileDir + "results/";
        TString inputFile2 = fin;
        fileName = inputFile2.Remove( 0, inputFile2.Last( '/')+1 );
