...
void writeHist_forFakeRate::SlaveBegin(TTree * /*tree*/)
{
	...
	histsForRegions<Double_t> jets_bScore_class{"jets_bScore", "HT of jets", 30, 500, 1500, jets_bScore};
	...
	histsForRegions<Double_t> jets_bScoreMultiply_class{"jets_bScoreMultiply", "square root of b score multiplicity", 10, 0, 1, jets_bScoreMultiply};
	histsForRegions<Double_t> jets_4largestBscoreSum_class{"jets_4largestBscoreSum", "sum of 4 largest jets b score", 10, 0, 4, jets_4largestBscoreSum};
	histsForRegions<Double_t> jets_4largestBscoreMulti_class{"jets_4largestBscoreMulti", "jets_4largestBscoreMulti", 10, 0, 1, jets_4largestBscoreMulti};
	histsForRegions<Double_t> bjetsM_invariantMass_class{"bjetsM_invariantMass", "bjetsM_invariantMass", 10, 0, 500, bjetsM_invariantMass};
...
Bool_t writeHist_forFakeRate::Process(Long64_t entry)
{
	...
	Bool_t is1tau0lCRb = *tausT_number == 1 && *leptonsMVAT_number == 0 && *jets_number < 8 && *bjetsM_num == 1;
	Bool_t is1tau0lCRbLTau = isTauLNum && *leptonsMVAT_number == 0 && *jets_number < 8 && *bjetsM_num == 1;
	...
	if (!m_isData)
	{
		...
		Bool_t is1tau0lCRbGen = *tausT_number == 1 && *tausT_genTauNum == 1 && *leptonsMVAT_number == 0 && *jets_number < 8 && *bjetsM_num == 1;
		Bool_t is1tau0lCRbLTauGen = isTauLNum && isTauLNumGen && *leptonsMVAT_number == 0 && *jets_number < 8 && *bjetsM_num == 1;
