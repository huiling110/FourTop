...
**Created**: 2025-12-11
**Last Updated**: 2025-12-11 (Session 4)
**Status**: IN PROGRESS - Starting 2017 Processing

**Session 4 Progress (2025-12-11)**:
- ✅ Documented shared OS/MV data in workflow skills (overview.md)
- ✅ Verified OS status for 1tau1l and 1tau0l (3 years complete: 2017, 2018, 2016preVFP)
- ✅ MV systematics submitted for 2017 1tau1l (14 variations, 89 jobs running)
- ✅ OS systematics submitted for 2016postVFP 1tau0l (workflow state tracking verified working)
- ✅ Updated context.md with comprehensive OS/MV status for all eras
- ⚠️ **IMPORTANT LESSON**: WH submission correctly blocked - must wait for MV jobs to complete (dependency chain: OS → MV → WH)
- 🔄 **NEXT**: Wait for 2017 MV jobs to finish, then submit WH nominal + systematics
...
## Session 4: Workflow State Validation & 2017 Processing (2025-12-11)

### Overview
Continuation session focusing on validating WorkflowState tracking, documenting shared OS/MV data structure, and starting 2017 processing for 1tau1l.

### Key Discovery: Shared OS/MV Data ⭐

**User insight**: "the 1tau1l and 1tau0l share the same OS and MV versions, remember this in workflow skill"

**Impact**: This is a critical efficiency insight - Stage 1 (OS) and Stage 2 (MV) data can be shared between 1tau1l and 1tau0l when using the same version strings (`v94HadroPreJetVetoHemOnly_TTBBtest`). Only Stage 3 (WH) and beyond differ between channels.

**Documentation updates**:
1. `.claude/skills/workflow/overview.md` - Added IMPORTANT note in "Available Configs" section
2. `dev-docs/active/workflow-optimization-v2/context.md` - Updated "Current State" with comprehensive OS/MV status table

### Stage 1 (OS) Status Verification ✅

Checked OS status for all eras (shared between 1tau1l and 1tau0l):

**✅ COMPLETE for 3 years**:
- **2017**: ✓ Nominal + 15 systematic variations (TES×8, JER×2, JES×1, MET×2, EleScale×2)
- **2018**: ✓ Nominal + 2 systematic variations (JERUp, JESPt22)
- **2016preVFP**: ✓ Nominal + systematics submitted

**🔄 IN PROGRESS**:
- **2016postVFP**: OS systematics being submitted now (workflow state tracking verification)

**Files checked**:
- `/publicfs/cms/user/huahuil/tauOfTTTT_NanoAOD/UL{ERA}/v94HadroPreJetVetoHemOnly_TTBBtest/mc/`
- Used script: `/tmp/check_os_ttbb.sh` (checks both nominal and systematic directories)

**Workflow state verification**:
- Submitted OS systematics for 2016postVFP using: `./submit_all_systematics.sh ../../config/analysis_config_1tau0l_TTBBtest.yaml 2016postVFP`
- Confirmed `.workflow_state.json` updated in real-time:
  - Current: `stage: 1.1, era: 2016postVFP, status: submitting, operation: OS_JERUp`
  - Stage 1.1 status now includes: `2016postVFP: submitting` ✅
- **Conclusion**: WorkflowState tracking working correctly!

### Stage 2.1 (MV Systematics) for 2017 ✅

**Task**: Submit MV systematic variations for 2017 to prepare for WH processing

**Command**:
```bash
cd makeVariables_goodCode/jobs
python3 makeJob_MV_JESVariation.py --config ../../config/analysis_config_1tau1l_TTBBtest.yaml --era 2017 --group all
```

**Results**:
- ✅ Submission completed successfully
- 14 systematic variation directories created
- 89 jobs currently running in queue
- Systematic variations: TES (dm0, dm1, dm10, dm11) × Up/Down, JER × Up/Down, MET × Up/Down, EleScale × Up/Down

**Directories created**:
```
/publicfs/cms/user/huahuil/tauOfTTTT_NanoAOD/forMVA/2017/v1baselineHadro_v94HadroPreJetVetoHemOnly_TTBBtest_{TESdm0Down,TESdm0Up,TESdm1Down,TESdm1Up,TESdm10Down,TESdm10Up,TESdm11Down,TESdm11Up,JERDown,JERUp,METDown,METUp,EleScaleDown,EleScaleUp}/mc/
```

**Workflow state update**:
- Current: `stage: 2.1, era: 2017, status: submitting, operation: MV_JESPt22`
- Todo updated: Task 1 marked completed, Task 2 (wait for MV) now in_progress

### Important Lesson: WH Dependency Chain ⚠️

**Issue**: Started submitting WH nominal for 2017, but user interrupted: "do not submit WH for 2017 as it depends on MV!"

**Correct dependency chain**: OS → MV → WH
- Stage 1 (OS) must complete before Stage 2 (MV) can start
- Stage 2 (MV) must complete before Stage 3 (WH) can start
- Stage 2.1 (MV systematics) must complete before Stage 3.1 (WH systematics) can start

**Why**: WH reads MV output ROOT files. If MV jobs are still running, WH will:
- Miss files (jobs not yet finished)
- Read incomplete data
- Fail or produce wrong results

**Corrective action**:
- Interrupted WH submission (user stopped it mid-execution)
- Updated todo list to reflect: "Wait for 2017 MV jobs to complete" as current task
- Will monitor `hep_q -u $USER | grep MV_2017` until all 89 jobs finish

### Files Modified (Session 4)
1. `.claude/skills/workflow/overview.md` - Added shared OS/MV data note
2. `dev-docs/active/workflow-optimization-v2/context.md` - Comprehensive OS/MV status update
3. `dev-docs/active/workflow-optimization-v2/tasks.md` - This file (Session 4 added)

### Next Steps (Session 4)

**Immediate** (waiting on):
- Monitor 2017 MV jobs completion (~89 running)
- When all MV jobs done: Submit WH nominal + systematics for 2017

**Pending for 2017**:
- Stage 3: WH nominal (`makeJob_WH.py --systematic nominal`)
- Stage 3.1: WH systematics (`makeJob_WH.py --systematic all`)
- Stage 4.1-4.4: Templates, datacards, plots
- Stage 4.5: Combine (in screen session)

**Current Status Summary**:
- 1tau1l 2018: ✅ Complete (Session 2-3)
- 1tau1l 2017: 🔄 MV systematics running (89 jobs)
- 1tau0l 2016postVFP: 🔄 OS systematics submitting
- OS data: ✅ 3/4 years complete (2017, 2018, 2016preVFP)
