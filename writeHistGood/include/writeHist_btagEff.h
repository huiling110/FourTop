...
private:
    ...
    TH2D *m_h2D_jets_ptEta_b = new TH2D(m_processName + "_genB_de_jets_ptEta", "p_{T}^{jet}(GeV):|#eta|", std::size(bins) - 1, bins, std::size(binsEta) - 1, binsEta); // maybe should have the same binnig as that of SF provided by BTV
    TH2D *m_h2D_jets_ptEta_c = new TH2D(m_processName + "_genC_de_jets_ptEta", "p_{T}^{jet}(GeV):|#eta|", std::size(bins) - 1, bins, std::size(binsEta) - 1, binsEta); // maybe should have the same binnig as that of SF provided by BTV
    TH2D *m_h2D_jets_ptEta_l = new TH2D(m_processName + "_genL_de_jets_ptEta", "p_{T}^{jet}(GeV):|#eta|", std::size(bins) - 1, bins, std::size(binsEta) - 1, binsEta); // maybe should have the same binnig as that of SF provided by BTV
    TH2D *m_h2D_jets_ptEta_b_nu = new TH2D(m_processName + "_genB_nu_jets_ptEta", "jets_ptEta_genB_nu", std::size(bins) - 1, bins, std::size(binsEta) - 1, binsEta);
    TH2D *m_h2D_jets_ptEta_c_nu = new TH2D(m_processName + "_genC_nu_jets_ptEta", "jets_ptEta_genC_nu", std::size(bins) - 1, bins, std::size(binsEta) - 1, binsEta);
    TH2D *m_h2D_jets_ptEta_l_nu = new TH2D(m_processName + "_genL_nu_jets_ptEta", "jets_ptEta_genL_nu", std::size(bins) - 1, bins, std::size(binsEta) - 1, binsEta);
