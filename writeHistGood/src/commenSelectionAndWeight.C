...
Bool_t baselineSelection(event *event, const Bool_t isRun3, Bool_t is1tau2l)
{
    ...
    if (!isRun3){
        if(!is1tau2l){
            ...
            pass = event->jets_num.v() >= 2 && event->bjetsM_num.v() >= 1 && event->jets_HT.v() > 200. && event->lepTopMVAT_1pt.v()>25. && event->lepTopMVAT_2pt.v()>20.  && !event->lepTopMVAT_2ifZVeto.v();//leptonPt selection should be weighed lepton cone corrected pt for fakeLepton; done in createFakeLepton.py
        }
    }else{
        // std::cout << "not run 3\n";
