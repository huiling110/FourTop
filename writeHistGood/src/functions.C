...
void initializeHistVec(const std::vector<TString>& regionsForVariables, std::vector<std::shared_ptr<histForRegionsBase>>& histsForRegion_vec , TString m_processName,  event* e ){
    ...
    SP_d tausF_1pt_class = std::make_shared<histsForRegionsMap<Double_t>>("tausF_1pt", "p_{T}^{1st fakeble#tau}", m_processName, 40, 20, 100, regionsForVariables, &(e->tausF_1pt)); //!should be tauT variables in FT region
...
void getChannelSys(std::vector<TString>& sysRegions, TString region, TString era, Bool_t isFaketau){
    ...
    sysRegions.push_back(region + "_CMS_tttt_eff_hltLep_" + era + "Up");
    sysRegions.push_back(region + "_CMS_tttt_eff_hltLep_" + era + "Down"); 
