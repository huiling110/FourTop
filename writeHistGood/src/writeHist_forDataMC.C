...
void WH_forDataMC::Init()
{
    ...
    // regions for hists
    std::vector<TString> regionsForVariables = {"1tau0lSR",  "1tau0lVR", "1tau0lCR", "1tau0lMR", "1tau1lCR1", "1tau1lCR2", "1tau1lSR", "baseline", "1tau1lRSL", "1tau0lSRTest"};
...
void WH_forDataMC::LoopTree(UInt_t entry)
{
    ...
    for (UInt_t i = 0; i < allEvent; i++)
    {
        ...
        // Double_t basicWeight = e->EVENT_genWeight.v() * e->EVENT_prefireWeight.v() * e->PUweight_.v() * e->tauT_IDSF_weight_new.v() * e->elesTopMVAT_weight.v() * e->musTopMVAT_weight.v()* e->btagWPMedium_weight.v()* e->HLT_weight.v(); //!!!without HLT weight
