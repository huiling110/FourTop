...
jobVersionName = 'v84Pre1tau2lLepF2V3/'  # Set your desired job version name

def main():
    era = '2018'  # Set the desired era
    sumProToSkip = ['jetHT', 'BTagCSV', 'qcd']  # Processes to skip
...
def makeSubAllJobs(jobsDir, era):
    ...
    for ijob in os.listdir(jobsDir + era):
        if '.sh' in ijob:
            subAllProcessName.write("hep_sub -os CentOS7 -argu \"%{ProcId}\" -n 5000 " + ijob + "\n")
...
def makeJobsInDir(inputDir, outputDir, jobScriptsFolder, isData, era, isRun3, sumProToSkip=[]):
    ...
    for k in allProcesses:
        ...
        oneProcess = jobScriptsFolder + k + ".sh"
        kProcessDir = jobScriptsFolder + k + '/'
        if os.path.exists(kProcessDir):
            subprocess.run('rm -fr ' + kProcessDir, shell=True)
        uf.checkMakeDir(kProcessDir)

        with open(oneProcess, 'w') as sub_oneProcess:
            sub_oneProcess.write("cd " + kProcessDir + "\n\n")
            print('job sub script for kProcess is: ', oneProcess)

            sampleDir = inputDir + k + '/'
            ...
            for entry in os.listdir(sampleDir):
                ...
                if os.path.isfile(os.path.join(sampleDir, entry)):
                    ...
                    iSmallJobName = 'OS_' + era + k + '_' + smallFile + ".sh"
                    smallFilejob = kProcessDir + iSmallJobName
                    prepareCshJob(sampleDir, koutputDir, smallFilejob, entry)

                    logFile = kOutDirLog + smallFile + ".log"
                    errFile = kOutDirLog + smallFile + ".err"
                    sub_oneProcess.write("hep_sub -os CentOS7 -mem 8000 -argu \"%{ProcId}\" -n 5000 " + iSmallJobName + " -o " + logFile + " -e " + errFile + "\n")

            os.popen('chmod 777 ' + kProcessDir + "/*sh")

def prepareCshJob(inputDir, koutputDir, shFile, singleFile):
    with open(shFile, 'w') as subFile:
