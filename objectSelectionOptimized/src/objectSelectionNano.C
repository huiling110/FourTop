...
void objectSelection::EventLoop(const Bool_t iftauSel, const Bool_t preSelection, const Bool_t ifHLT, ULong_t numEntries)
{
    ...
    while (m_reader.Next() && entryCount < numEntries)
    {
        ...
        if(!(OS::ifEventPass(preSelection, jetSel.getSize()>1, m_cutflow, 4))){//!for 1tau2l
            continue;
        }
        // if(!(OS::ifEventPass(preSelection, bjetMSel.getSize()>1, m_cutflow, 5))){//baseline for 1tau1l and 1tau0l
        if(!(OS::ifEventPass(preSelection, bjetMSel.getSize()>0, m_cutflow, 5))){//baseline for 1tau2l
            continue;
