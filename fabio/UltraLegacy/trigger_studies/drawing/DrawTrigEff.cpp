...
void DrawTrigEff(string YEAR, TString VAR, TString CAT, bool LOG, bool plot_sig_only, TFile *outputfile, bool PRINT) {
...
while (xsec_it != xsec[YEAR].end()) {
    ...
    if (xsec_it == xsec[YEAR].begin()) {
        ...
        if (!(xsec_it->first.find(data) !=std::string::npos)) {
            ...
            //DATA->SetWeight(lumi[YEAR]/lumi_sec[xsec_it->first]);
...
if (PRINT) {//save plots
    can->SaveAs("plots_tryFixData/e_"+VAR+"_"+CAT+"_"+TString(YEAR)+".png"); 
  }
...
if (VAR == "HT" && CAT == "nocat" && !plot_sig_only) {
    ...
    F0->Write("SF_HT_"+CAT+"_"+TString(YEAR));
    DATA->Write("e_HT_nocat_DATA_"+TString(YEAR));
    MC->Write("e_HT_nocat_MC_"+TString(YEAR));  
}

if (VAR == "njets" && CAT == "nocat" && !plot_sig_only) {
      
    F0->Write("SF_njets_"+CAT+"_"+TString(YEAR));
    DATA->Write("e_njets_nocat_DATA_"+TString(YEAR));
    MC->Write("e_njets_nocat_MC_"+TString(YEAR));
